...
external MCP servers. The goal is to explore the following:
    ...
    5. Confirmation - Long-running operations: pause selected MCP tools and ask 
                      the human for approval. A defensive, deny-by-default approach 
                      with human in the loop policy is implemented as a plugin.
    ...
    7. Observability- cross-cutting telemetry implemented using ADK plugins.
...
--------------------------------
Memory (for long term knowledges) are handled separately than session:

    Session (SessionService)    -> Short-term
    Memory (MemoryService)      -> Long-term. A searchable knowledge store 
                                   spanning many sessions.
                                   
Memory lifecycle:

    1. Initialize   -> Pick a backend and hand it to the runner (--memory)
    2. Ingest       -> Copy the finished session into the store (--memory-save)
    3. Retrieve     -> Let the agent search the store (--memory-retrieval)
...
Retrieval (--memory-retrieval):
    ...
    none    -> (default) Memory is written but the agent never sees it.
    load    -> `load_memory` tool (reactive: the agent searches when it decides to).
    preload -> `preload_memory` tool (proactive: every turn is prefixed with a search).

Ingestion (--memory-save): when the memory should be saved

    every-turn  after_agent_callback fires each turn            -> real-time recall.
    end         one save when the chat loop exit                -> fewer write/calls.
    interval    a background asyncio task saves every N secs    -> long-running chats
                (tune N with --memory-interval)

How to use memory with agent (ex):
...
---------------------------------------
Long running operations and sensitive tools pause and ask a human for approval. 
This is enforced by `ConfirmationPolicyPlugin`, a global plugin registered once 
at the runner rather than a per-agent callback. Since plugins callbacks run before 
agent-level callbacks, all tools and agents managed by the runner will be gated by 
these plugins.

The policy is defensive:

    * deny-by-default   -> anything not explicitly bypassed is gated.
    * allowlist bypass  -> a tool skips confirmation only if its (server, tool) pair 
                           is allowed via `MCPServerSpec.bypass_confirmation`.
    * non-bypassable floor -> destructive verbs (delete/remove/force/push/...) always 
                           require confirmation, even if they are bypassed.
    * fail-closed       -> unknown tools (not seen at discovery) requires confirmation.
    
Observability (logs, metrics)
-------------------------------------
Telemetry is cross-cutting and has the following plugins:

    * MetricsPlugin - always on (except with --quiet). Emit one aggregate summary per 
                      invocation: latency, tool-call count, token usage (once per real 
                      model call), and any tool/model errors.
    * LoggingPlugin - ADK's built-in logger, attached only with --debug.
...
from typing import Any, AsyncIterator, Union, Callable, Awaitable, Iterator, Optional
...
DEFAULT_MEMORY_BACKEND = "off"              # off | memory | sqlite
DEFAULT_MEMORY_RETRIEVAL = "none"           # none | load | preload
DEFAULT_MEMORY_SAVE = "end"                 # every-turn | interval | end
DEFAULT_MEMORY_INTERVAL = 60.0              # seconds; only used with --memory-save interval
DEFAULT_MEMORY_DB_PATH = "chat_memory_db"   
DEFAULT_MEMORY_TOP_K = 10                   # Max ranked results returned by a memory search
...
class MCPServerSpec:
    ...
    Confirmation is deny-by-default. `bypass_confirmation` is an allowlist of 
    tool names on this server that may skip the gate. By is opt-in, never opt-out. 
    Newly discovered tools always require confirmation.
    
    Bypass can be overwritten by `is_destructive` method if one destructive word 
...
DESTRUCTIVE_TOKENS: frozenset[ str ] = frozenset({
    "delete", "remove", "destroy", "drop", "rm",
    "force", "push", "merge", "reset", "revert"
})

def is_destructive( tool_name: str ) -> bool:
    """Check if tool_name contains destructive words."""
...
class ConfirmationPolicy:
    """Per tool, whether a human must approve before a tool runs.
    
    Tools are namespaced by server (`tool_name_prefix`). Defensive by design:
    
        1. destructive floor    -> always confirm (no bypass).
        2. ambiguous name       -> confirm if servers shaed a name or overlape 
                                   namespaces.
        3. unknown              -> confirm if not seen at discovery.
        4. bypass allowlist     -> skip confirmation for an opted-in (server, tool)
        5. otherwise            -> confirm (deny-by-default).
    """
    
    # (server, prefixed_tool) pairs explicitly allowed.
    bypass: frozenset[ tuple[ str, str ] ]
    # Prefixed runtime tool name -> owning server name.
    tool_to_server: dict[ str, str ]
    # Prefixed runtime tool name -> server's own raw tool name.
    runtime_to_raw: dict[ str, str ]
    # For (server, tool) leading to the same runtime names. Gated.
    ambiguous: frozenset[ str ]
    
    def requires_confirmation( self, tool_name: str ) -> bool:
        # 1. Destructive floor
        raw = self.runtime_to_raw.get( tool_name, tool_name )
        if is_destructive( raw ):
            return True
        
        # 2. Ambiguous
        if tool_name in self.ambiguous:
            return True
        ...
        return ( server, tool_name ) not in self.bypass
        
def build_confirmation_policy(
    specs: list[ MCPServerSpec ], discovered: dict[ int, list[ str ] ]
) -> ConfirmationPolicy:
    """Compile the declarative list of MCP servers config + discovered tools into policy."""
    ...
    for position, spec in enumerate( specs ):
        for raw in discovered.get( position, [] ):
            ...
            # Check if a server already uses the same runtime name. If so, flag it as ambiguous.
            owner = tool_to_server.get( runtime_name )
            if owner is not None and owner != spec.name:
                ambiguous.add( runtime_name )
                log.warning(
                    f"confirmation.ambiguous tool={runtime_name!r} servers={owner!r}, {spec.name!r}"
    ...
    # Log at startup what will and won't be gated
    bypassed = sorted( f"{server}:{tool}" for server, tool in bypass )
    gated = sorted(
        f"{server}:{tool}"
        for tool, server in tool_to_server.items()
...
class ConfirmationPolicyPlugin( BasePlugin ):
    """Global human-in-the-loop gate, enforced by a plugin.
    
    Registered at the Runner so it applies to every agent/tool. It cannot be bypassed by per-agent code.
    """
    ...
    async def before_tool_callback(
        ...
        if not self.policy.requires_confirmation( tool.name ):
            log.info( f"confirmation.bypass tool={tool.name!r} (per policy)" )
            return None
        ...
        if confirmation is None:
            ...
            log.info( f"confirmation.request tool={tool.name!r}" )
            return { "status": "pending", "message": f"Awaiting approval for {tool.name!r}" }
        
        if confirmation.confirmed:
            log.info( f"confirmation.approved tool={tool.name!r}" )
            return None
...
@dataclass
class RunMetrics:
    """Mutable accumulator for a single invocation."""
...
class MetricsPlugin( BasePlugin ):
    """Pure-observation Plugin: collect the metrics and emits one aggregate 
    summary per invocation.
    
    Every hook returns None so the pluing does not alter control flow by bypassing 
    other callbacks. State metrics is keyed per invocation_id to handle concurrency.
    """
    ...
    async def on_tool_error_callback(
        ...
        self._run( tool_context.invocation_id ).tool_errors += 1
        log.warning( f"tool.error tool={tool.name!r} err={error!r}")
        return None
    
    async def on_model_error_callback(
        self, *, callback_context: CallbackContext, llm_request: LlmRequest, error: Exception
    ) -> None:
        self._run( callback_context.invocation_id ).model_errors += 1
        log.warning( f"model.error err={error!r}")
        return None
    ...
    async def after_run_callback(
        ...
        if run is None:
            return None
        elapsed_ms = ( time.perf_counter() - run.start ) * 1000
        log.info(
            f"metrics inv={invocation_id} latency_ms={elapsed_ms:.0f} "
            f"tool_calls={run.tool_calls} tool_errors={run.tool_errors} "
            f"model_errors={run.model_errors} "
...
async def chat_loop(
    ...
    while True:
        ...
        try:
            ...
            log.exception( f"turn.error user={user_id} session={session_id}" )
...
class MemoryBackend( str, Enum ):
    """Where backend memory lives."""
    
    OFF = "off"         # No memory.
    MEMORY = "memory"   # InMemoryMemoryService: RAM only, keyword search.
    
class MemoryRetrieval( str, Enum ):
    """How the agent reads memory."""
    
    NONE = "none"       # Memory is written but the agent never reads it.
    LOAD = "load"       # Load memory is provided to the agent as a tool, reactive approach.
    PRELOAD = "preload" # The agent preload memory in each turn.

class MemorySaveTrigger( str, Enum ):
    """How often the agent writes into memory."""
    
    EVERY_TURN = "every-turn"   # After every turn, the agent write in memory using `after_agent_callback`
    END_OF_CONVERSATION = "end" # One save when exiting the chat
    INTERVAL = "interval"       # background asyncio task
    
@dataclass
class MemoryConfig:
    """Long-term memory policy."""
    
    backend: MemoryBackend = MemoryBackend.OFF
    ...
    @property
    def enabled( self ) -> bool:
        return self.backend is not MemoryBackend.OFF
    
    def __post_init__( self ):
        if not self.enabled:
            return
        if self.save_trigger is MemorySaveTrigger.INTERVAL and self.save_interval_seconds <= 0:
            raise ValueError( "save_interval_seconds must be > 0 for interval saving" )
    ...
    def describe( self ) -> str:
        ...
        when = (
            f"every {self.save_interval_seconds:g}s"
            if self.save_trigger is MemorySaveTrigger.INTERVAL
            else self.save_trigger.value
        )
        return (
            f"memory {self.backend.value};"
            f"retrieval={self.retrieval.value}; save={when}"
        )
        
def build_memory_service( config: MemoryConfig ) -> tuple[ BaseMemoryService | None , str ]:
    """Construct the memory service for the chosen backend.
    
    Returns (service, label). `None` means no memory.
    
    For now, it only provides in-memory memory (RAM only) with no persistence. Memory 
    is therefore lost on exit and is therefore binded to each session.
    
    A production implementation would handle durability, semantic search, and consolidation. 
    Any other memory service (e.g., VertexAiMemoryBankService) should implement a `BaseMemoryService` 
    interface.
    """
    
    if not config.enabled:
        return None, "disabled"
    return InMemoryMemoryService(), "in-memory (keyword search, lost on exit)"

def build_memory_tools( config: MemoryConfig ) -> list[ BaseTool ]:
    """Return the retrieval tool to add to the agent.
    
    Production: this should probably be adapted to load and preload from VertexAI.
    """
    
    if not config.enabled or config.retrieval is MemoryRetrieval.NONE:
        return []
    if config.retrieval is MemoryRetrieval.PRELOAD:
...
def build_app(
    agent: Agent, 
    compaction: EventsCompactionConfig | None = None,
...
async def chat(
    ...
    async with mcp_toolsets( specs ) as toolsets:
        ...
        # Compile the confirmation policy from the declarative server specifications.
        confirmation_policy = build_confirmation_policy( specs, discovered )
        ...
        # Plugins registered once on the Runner and apply to every agent/tool.
        # Order matters: LoggingPlugin first such that it traces the tool attemps before 
        # the policy decides.
        plugins: list[ BasePlugin ] = []
        if debug:
            plugins.append( LoggingPlugin() )
        plugins.append( ConfirmationPolicyPlugin( confirmation_policy ) )
        plugins.append( MetricsPlugin() )
        
        app = build_app( agent, compaction_config, plugins )
        ...
        log.info(
            f"Startup app={APP_NAME} agent={agent.name} model={MODEL}" 
...
def configure_logging( debug: bool, quiet: bool ) -> None:
    """Set output verbosity:
        ...
        --debug -> app DEBUG. The full logs from LoggingPlugin which prints on 
                   stdout independently of these levels.
...
def main() -> None:
    ...
    memory_group.add_argument(
        ...
        help = (
            "How the agent reads memory: 'none' (default; write-only), "
            "'load' (reactive load_memory tool), or 'preload (proactive "
    ...
    memory_group.add_argument(
        ...
        help = (
            "When to copy the session into memory: 'every-turn' "
            "(after_agent_callback), 'end' (default; on chat exit), or "
    ...
    memory_group.add_argument(
        ...
        help = f"Seconds between consecutive background saves for --memory-save interval. Default:{DEFAULT_MEMORY_INTERVAL:g}"
    )
    ...
    asyncio.run( chat( config, compaction_settings, memory_config, debug = args.debug ) )
